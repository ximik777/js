...
createChildClass('DropdownMenu', UiControl, {
    ...
    addItem: function(item) {
        ...
        link.innerHTML = (this.options.checkable ? '<img src="/temp/images/dropdead_check.gif">' : '') + item.l;
...
createChildClass('Slider', UiControl, {
    ...
    'initOptions': function(input, options)
    {
        ...
        if(!this.data)
        {
            ...
            if(opts.min>=opts.max){return false;}
        }
    ...
    'initEvents': function()
    {
        ...
        var shift_x = 0, p_d = 0, candrag = false;
    ...
    'initDOM': function ()
    {
        ...
        for(var i= 0; i<self.temp_arr.length; i++)
        {
            ...
            if(i==0)
            {
                self.p_arr[i] = {'value': self.temp_arr[i], 'point': p, 'lborder': p, 'rborder': p};
            }
            else
            {
                self.p_arr[i] = {'value': self.temp_arr[i], 'point': p, 'lborder': Math.round((self.p_arr[i-1]['point'] + p) / 2), 'rborder': p};
    ...
    {
        step = step - 1;
        var foo = [], t = Math.round(max/step), k = 0;
    ...
    'afterInit': function()
    {
        if(this.disabled===true){this.disable(true);}
        if(this.options.value_area)
        {
            this.value_area.innerHTML = this.value+' '+this.options.value_area_prefix;
        }
